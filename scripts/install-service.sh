...
if $WRITE_CONFIG; then
  ...
  # Download the example config as the starting point (has all options documented).
  EXAMPLE_URL="https://raw.githubusercontent.com/$REPO/main/golinx.example.toml"
  if command -v curl >/dev/null 2>&1; then
    curl -fsSL -o "$CONFIG_FILE" "$EXAMPLE_URL"
  else
    wget -q -O "$CONFIG_FILE" "$EXAMPLE_URL"
  fi
  ...
  if [ -n "$TS_HOSTNAME" ]; then
    sed -i "s/^ts-hostname = .*/ts-hostname = \"$TS_HOSTNAME\"/" "$CONFIG_FILE"
  fi
